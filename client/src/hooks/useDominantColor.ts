...
const useDominantColor = (imageUrl: string | null) => {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      if (!ctx) return setColors({ bg: DEFAULT_BG, text: DEFAULT_TEXT });
      ...
      let dominant = [...colorMap.entries()].reduce((max, curr) => (curr[1] > max[1] ? curr : max))[0];
      let [r, g, b] = dominant.split(",").map(Number);

      let [bgR, bgG, bgB] = adjustIfTooDark(...blendWithBlack(r, g, b, 0.5));
      let [textR, textG, textB] = ensureTextContrast(bgR, bgG, bgB, ...blendWithWhite(r, g, b, 0.7));

      setColors({
        bg: `rgba(${bgR},${bgG},${bgB},0.9)`,
        text: `rgb(${textR},${textG},${textB})`,
      });
    };

    img.onerror = () => setColors({ bg: DEFAULT_BG, text: DEFAULT_TEXT });
  }, [imageUrl]);
